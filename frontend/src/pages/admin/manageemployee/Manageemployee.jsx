...
const Manageemployee = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("http://localhost:3500/api/employee/allemployee");
  ...
  const handleDelete = async (id) => {
    try {
     const empdelete= await axios.delete(`http://localhost:3500/api/employee/delete/${id}`);
        ...
        <tbody>
          {employees.map((employee,index) => (
            <tr key={employee._id}>
              ...
              <td className="border px-4 py-2">{employee.emptype}</td>
