...
const Updatewarden = () => {
    ...
    useEffect(() => {
        ...
        const fetchEmployeeData = async () => {
            try {
                const response = await axios.get(`http://localhost:3500/api/warden/managewarden/${id}`);
                const wardenData = response.data;
                // Set form data with employee data
                setFormData(wardenData);
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(`http://localhost:3500/api/warden/update/${id}`, formData);
            const responseData = response.data;
            if (responseData.status === "pass") {
                // Handle success, maybe redirect or show a success message
                toast.success("Warden updated successfully");
                navigate("/managewarden")
